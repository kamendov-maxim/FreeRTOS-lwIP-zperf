...
#define ZPERF_WORK_Q_THREAD_PRIORITY                                                                                   \
    CLAMP(CONFIG_ZPERF_WORK_Q_THREAD_PRIORITY, K_LOWEST_APPLICATION_THREAD_PRIO, K_HIGHEST_APPLICATION_THREAD_PRIO)
static K_THREAD_STACK_DEFINE(zperf_work_q_stack, CONFIG_ZPERF_WORK_Q_STACK_SIZE);

static k_work_q zperf_work_q;
...
int zperf_get_ipv4_addr(char *host, struct in_addr *addr)
{
    ...
    ifaddr = net_if_ipv4_addr_add(net_if_get_default(), addr, NET_ADDR_MANUAL, 0);
    if (!ifaddr)
    {
...
int zperf_prepare_upload_sock(const struct sockaddr *peer_addr, int tos, int priority, int proto)
{
    ...
    case AF_INET:
        ...
        if (sock < 0)
        {
            NET_ERR("Cannot create IPv4 network socket (%d)", errno);
            return -errno;
        }

        if (tos > 0)
        {
            if (zsock_setsockopt(sock, IPPROTO_IP, IP_TOS, &tos, sizeof(tos)) != 0)
            {
    ...
    case AF_INET6:
        ...
        if (sock < 0)
        {
            NET_ERR("Cannot create IPv6 network socket (%d)", errno);
            return -errno;
        }
    ...
    default:
        NET_ERR("Invalid address family (%d)", peer_addr->sa_family);
        return -EINVAL;
    }
    ...
    if (IS_ENABLED(CONFIG_NET_CONTEXT_PRIORITY) && priority >= 0)
    {
        ...
        if (!IS_ENABLED(CONFIG_NET_ALLOW_ANY_PRIORITY) && (prio >= NET_MAX_PRIORITIES))
        {
            NET_ERR("Priority %d is too large, maximum allowed is %d", prio, NET_MAX_PRIORITIES - 1);
            return -EINVAL;
        }

        if (zsock_setsockopt(sock, SOL_SOCKET, SO_PRIORITY, &prio, sizeof(prio)) != 0)
        {
...
int udp_uploader_run()
{
    ...
    char *port_str = DEF_PORT_STR;
    int start = 0;
    size_t opt_cnt = 0;

    int ret = zperf_udp_upload(&param, &results);
    if (ret < 0)
    {
