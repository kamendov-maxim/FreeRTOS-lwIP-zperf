...
static struct sockaddr_storage tcp_server_addr;
...
static int tcp_bind_listen_connection(zsock_pollfd *pollfd,
...
static void tcp_server_session(void)
{
	...
	static zsock_pollfd fds[SOCK_ID_MAX];
	static struct sockaddr_storage sock_addr[SOCK_ID_MAX];
	int ret;
	...
	if (IS_ENABLED(CONFIG_NET_IPV4)) {
		...
		addr = &net_sin((struct sockaddr*)(&tcp_server_addr))->sin_addr;
...
use_any_ipv4:
	    ...
	    copy_sockaddr_in_to_sockaddr_storage(in4_addr, &sock_addr[SOCK_ID_IPV4_LISTEN]);
		

		ret = tcp_bind_listen_connection(
				&fds[SOCK_ID_IPV4_LISTEN],
				((struct sockaddr*)&sock_addr[SOCK_ID_IPV4_LISTEN]));
		if (ret < 0) {
			goto error;
		}
	}
	...
	if (IS_ENABLED(CONFIG_NET_IPV6)) {
		...
		addr = &net_sin6((struct sockaddr*)(&tcp_server_addr))->sin6_addr;
...
use_any_ipv6:
	    ...
	    copy_sockaddr_in6_to_sockaddr_storage(in6_addr, &sock_addr[SOCK_ID_IPV6_LISTEN]);

		ret = tcp_bind_listen_connection(
				&fds[SOCK_ID_IPV6_LISTEN],
				(struct sockaddr*)(&sock_addr[SOCK_ID_IPV6_LISTEN]));
		if (ret < 0) {
			goto error;
		}
	}
	...
	while (true) {
		...
		for (int i = 0; i < ARRAY_SIZE(fds); i++) {
			...
			if ((i >= SOCK_ID_IPV4_LISTEN) && (i <= SOCK_ID_IPV6_LISTEN)) {
				...
				struct sockaddr_storage addr_incoming_conn;
				...
				tcp_received((struct sockaddr*)(&sock_addr[i]), ret);
...
void tcp_receiver_thread(void *ptr1)
{
	ARG_UNUSED(ptr1);

	k_sem_init(&tcp_server_run,
	           tcp_server_run.initial_count,
			   tcp_server_run.max_count);
